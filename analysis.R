...
# Helper for saving files to disk
save_plot <- function(path, plot) {
  # if (!file.exists(path)) 
	ggsave(path, plot, width = 7, height = 4.5, dpi = 150)
}
...
save_plot("images/log_insurance_charges_vs_sex_distribution.png", log_sex_plot)
...
save_plot("images/age_vs_sex_distribution.png", age_vs_sex_plot)
...
save_plot("images/age_vs_log_charges_distribution.png", age_vs_log_charges_plot)

bmi_vs_sex_plot <- 
	ggplot(df, aes(x=sex, y=bmi, color=sex)) + 
  geom_boxplot(alpha = 0.7, outlier.size = 0.8) +
	labs(title="Age Vs. Sex", x="sex", y="BMI (kg/m^2)")
save_plot("images/bmi_vs_sex_distribution.png", bmi_vs_sex_plot)
...
save_plot("images/bmi_vs_log_charges_distribution.png", bmi_vs_log_charges_plot)
...
if (file.exists("tables/regression_table.tex")) file.remove("tables/regression_table.tex")
...
writeLines(table_lines, "tables/regression_table.tex")
